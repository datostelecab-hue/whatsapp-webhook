...
- 🚨 **`mileage` NO es el odómetro del coche.** Son los km recorridos DESDE QUE SE INSTALÓ EL DISPOSITIVO. Contrastado el 09/09/2026 contra los km de la última revisión que lleva el taller: de 27 coches con los dos datos, **25 daban un imposible** (5886LBZ: `mileage` 30.723 contra 629.100 km reales). Confirmado además por los datos: en 87 de 95 unidades el `mileage` equivale a entre 1 y 4 meses de su propio ritmo (mediana 2,3 — la flota se instaló en verano de 2026), y una unidad recién dada de alta marca casi cero. Sirve para medir recorrido, no para saber por dónde va un coche.
- ✅ **El odómetro de verdad está en `include[]=can` → `can.odom`**, en KILÓMETROS con decimal y con su propio `gmt`. Es la lectura del CAN bus, o sea el número del cuadro. Contrastado contra los mismos 26 coches: **26 de 26 cuadran, ninguno imposible.** Lo dan 90 de las 144 unidades (las demás llevan un GPS que no lee el CAN). El bloque `can` trae también `fuel_total`, `can_fuel` y `engine_rpm_avg`. Lo usa `mapon.unidades()` → `odometroCanM`, y de ahí sale `vehiculo.km_odometro_m`.
- ⚠️ **`state` es un OBJETO** `{name, start, duration, debug_info}`, no una cadena; hay además un `movement_state` aparte. Tratarlo como texto daba `[object Object]` en las 144 unidades.
...
- **`unit/change_relay.json` — corte de motor. ✅ YA FUNCIONA (16/09/2026): Mapon concedió el permiso y se probó en el coche.** `POST`, parámetros: `key`, `unit_id`, `relay_id`, `relay_state` (bool 0/1). **Verificado en la flota (agosto 2026):** los coches SÍ llevan relé — `relay_id: 1`, `type: engine_block`, título "Bloqueo Motor", `enabled: 1`, `inverted: 0`, `control_while_moving: 0`. Semántica comprobada en coche real: **0 = motor libre, 1 = motor bloqueado**.
  - ⚠️ **Requiere permisos especiales que solo concede el soporte de Mapon.** Literal de la doc: *"special access rights are required to use this endpoint. Contact the support team to grant access"*. Sin ellos responde el error **global 1006 "Method not available"** (los errores propios del método son del 1 al 7). No se arregla en los ajustes de la API key. **Concedido ya.**
  - ⚠️ **El 1006 NO distingue "sin permiso" de "esa ruta no existe".** Comprobado el 16/09/2026: `unit/change_relay_list.json`, que es inventada, devuelve el mismo 1006. Así que un 1006 nunca prueba por sí solo que un método exista.
  - 🚗 **PROBADO EN EL COCHE (16/09/2026, 7222LVG / unit 893958, Toyota Corolla).** Se manda el 1, se relee y en **dos segundos** el coche lo confirma. Con el corte puesto **no arranca**: el bloqueo es real, no un estado en el servidor.
  - 🚨 **NUNCA CORTAR CON EL CONTACTO PUESTO.** Si el relé entra con el coche encendido, el Corolla arranca y anda **pero ya no se deja apagar**: el corte está metido en la línea que el coche necesita para completar el apagado, y el conductor se queda con un coche encendido que no responde al botón. Es peor que no bloquearlo. Por eso el fichaje no deja terminar turno con el contacto puesto (`puedeInmovilizar` en `services/fichaje.js`).
...
- 🔌 **La ignición NO viene como 0/1: viene como `{ gmt, value: 'on'|'off' }`.** `!!Number(u.ignition)` sobre ese objeto da NaN, o sea `false` — y el sistema se pasó semanas creyendo que ningún coche tenía nunca el contacto puesto. Se interpreta con `contactoPuesto()` en `services/mapon.js`, que devuelve `null` cuando no lo sabe: "no lo sé" no puede leerse como "no".
...
- 📞 **El teléfono es único ENTRE CONDUCTORES Y USUARIOS A LA VEZ.** `driver/create` con un teléfono que ya usa un **usuario** de la plataforma devuelve `error 1002: The phone has already been taken` — y ese usuario **no aparece en `driver/list`**, así que buscarlo allí no lo encuentra nunca. Pasó con el +34640389649 (usuario `datostelecab@gmail.com`) y dejó cuatro turnos sin enlazar el 16/09/2026. Los usuarios se listan con `user/list.json`. El fichaje, al chocar con el 1002, crea el conductor **sin teléfono**: el nombre sobre el coche vale más que la ficha completa.
- `driver/list.json` devuelve `id, name, surname, email, phone, language, ibutton, tacho, access_to_route_sheet, blocked, created`. **No pagina** (`limit`/`offset`/`page` se ignoran: siempre la lista entera).
