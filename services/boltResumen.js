...
async function actualizarTodo() {
  ...
  for (let d = 1; d <= diaActual; d++) {
    if (!diasCalculados.includes(d)) {
      diasFaltantes.push(d);
    }
  }

  console.log(`📊 Días ya en caché: ${diasCalculados.length} | Días a consultar: ${diasFaltantes.length}`);

  // 2. Si faltan días, consultar a Bolt SOLO para esos días
  let horasWaiting = 0;
  let horasHasOrder = 0;
  ...
  if (diasFaltantes.length > 0) {
    ...
    // Consultar stateLogs SOLO para días faltantes
    for (const flota of flotas) {
      ...
      for (const logs of Object.values(logsByDriver)) {
        ...
        for (let i = 0; i < logs.length; i++) {
          ...
          // Solo procesar días que estaban pendientes
          if (!diasFaltantes.includes(dia)) continue;

          if (!cache[dia]) cache[dia] = { flota63530: 0, flota143626: 0 };
    ...
    // Recalcular waiting/has_order desde TODA la caché
    for (const datos of Object.values(cache)) {
      // Aproximación: todo es has_order (no tenemos el desglose por día en caché)
      horasHasOrder += (datos.flota63530 + datos.flota143626);
    }
    horasHasOrder = horasHasOrder / 3600;

    // Consultar facturación SOLO para días nuevos
    ...
    // Sin días nuevos, calcular desde caché
    for (const datos of Object.values(cache)) {
      horasHasOrder += (datos.flota63530 + datos.flota143626);
    }
    horasHasOrder = horasHasOrder / 3600;
  ...
  // Leer facturación anterior
  const facturacionAnterior = await leerFacturacionAcumulada();
  const facturacionTotal = facturacionAnterior + facturacionNueva;

  console.log(`💰 Facturación: anterior=${facturacionAnterior.toFixed(2)} + nueva=${facturacionNueva.toFixed(2)} = ${facturacionTotal.toFixed(2)}`);
