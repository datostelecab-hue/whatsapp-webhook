    ...
    for (const flota of flotas) {
        ...
        for (const logs of Object.values(logsByDriver)) {
            ...
            for (let i = 0; i < logs.length; i++) {
                ...
                // Solo contar si hay un siguiente log (MISMA LÓGICA QUE HORAS_POR_DIA)
                if (i + 1 >= logs.length) continue; // ← AÑADIR ESTO

                const dia = new Date(logs[i].created * 1000).getDate();
                const inicio = logs[i].created;
                const fin = logs[i + 1].created; // ← SIEMPRE el siguiente log
