-- ============================================================
-- 18 - REPONE LA 17, QUE SE EDITO DESPUES DE APLICARSE
-- ============================================================
-- La 17 se aplico y DESPUES se le siguieron añadiendo cosas: `previo_alta` en
-- los tipos de documento, los reversos del DNI y del carne, `inicio_previsto`,
-- `jornada_horas` y `tipo_contrato` en la candidatura, y los campos de la
-- persona en `v_candidatura`. En la base quedo la version vieja.
--
-- UNA MIGRACION APLICADA ES INMUTABLE. Lo que hay que corregir va en un fichero
-- nuevo, y esto es ese fichero.
--
-- Todo lo de aqui se puede ejecutar aunque ya este hecho: las tablas van con IF
-- NOT EXISTS, los catalogos con ON CONFLICT DO UPDATE y las columnas con ADD
-- COLUMN IF NOT EXISTS. Si la 17 se aplico entera, esto no cambia nada.

BEGIN;

-- Las vistas se tiran y se rehacen, en orden de dependencia: `v_documento_falta`
-- cuelga de `v_documento_falta_persona`, asi que cae primero. Rehacerlas es la
-- unica forma de cambiar el orden de sus columnas, que es justo lo que cambio.
DROP VIEW IF EXISTS v_documento_falta;
DROP VIEW IF EXISTS v_documento_falta_persona;
...
-- La tabla puede existir ya, de la 17 aplicada. `CREATE TABLE IF NOT EXISTS` la
-- deja como esta y NO añade columnas, asi que las que se sumaron despues van
-- una por una. Si ya estan, no pasa nada.
ALTER TABLE IF EXISTS candidatura ADD COLUMN IF NOT EXISTS inicio_previsto DATE;
ALTER TABLE IF EXISTS candidatura ADD COLUMN IF NOT EXISTS jornada_horas   SMALLINT;
ALTER TABLE IF EXISTS candidatura ADD COLUMN IF NOT EXISTS tipo_contrato   VARCHAR(20);
...
CREATE VIEW v_documento_falta_persona AS
