 ...
 * El panel entero.
 *
 * LO QUE HACE CADA COCHE AHORA sale de la FOTO DEL AHORA
 * (services/flotaViva/ahora.js), la misma que lee el mapa: ya trae el tramo del
 * motor y el último apunte de BOLT cruzados, con la noticia más fresca elegida
 * y los empates resueltos. Hasta el 25/09/2026 esto hacía su propia consulta
 * con su propia copia de la regla, y el mapa y Control decían cosas distintas
 * del mismo coche. Aquí solo se reparte.
 *
 * Se trae todo de una vez y se reparte aquí en vez de lanzar tres consultas:
 * partirlo solo añade la posibilidad de que las partes no cuadren con el total.
 */
async function estado() {
  const foto = await ahora.foto();
  // Solo los vigilados (`fv_ahora`), como siempre: la foto trae también los
  // coches que BOLT conoce y no se vigilan, que son del mapa.
  const orden = x => (x.situacion_orden == null ? 99 : Number(x.situacion_orden));
  const todas = foto.coches.filter(x => x.vigilado)
    .sort((a, b) => ((b.conectado === true) - (a.conectado === true)) || (orden(a) - orden(b)) ||
      ((Number(b.segundos) || 0) - (Number(a.segundos) || 0)) || String(a.matricula).localeCompare(String(b.matricula)))
    .map(fila);
