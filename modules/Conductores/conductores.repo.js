...
const bolt = require('./cazamiento.repo');
...
/**
 * EL PIN DE LA TARJETA DE COMBUSTIBLE. Se lo pone Administración.
 *
 * Vive en la PERSONA y no en su candidatura (db/124): se le asigna a
 * cualquiera —tenga proceso de selección o no—, cambia con el tiempo y se
 * reenvía. Nada de eso tiene que ver con cómo entró hace ocho meses.
 *
 * `obs` a null NO borra la nota: quiere decir «no la toques». Corregir un PIN
 * sin querer tocar la observación es lo que se hace la mitad de las veces.
 */
async function guardarPinBallenoil(id, { pin, obs } = {}, { usuarioId } = {}) {
  const p = String(pin == null ? '' : pin).trim();
  const antes = (await db.consulta('SELECT pin_ballenoil FROM conductor WHERE id = $1', [Number(id)])).rows[0];
  ...
  const r = await db.consulta(
    `UPDATE conductor
        SET pin_ballenoil = $2,
            ...
            actualizado_at = now()
      WHERE id = $1
      RETURNING id, pin_ballenoil, obs_ballenoil,
                COALESCE(NULLIF(btrim(nombre_bolt), ''),
                         btrim(COALESCE(apellidos || ', ', '') || nombre)) AS quien`,
    [Number(id), p.slice(0, 40), obs == null ? null : String(obs).slice(0, 500)]);
  if (!r.rowCount) throw new Error('No existe esa persona');
  // El cambio queda apuntado como cualquier otro de la ficha: el PIN es de
  // quien reposta y saber quién se lo cambió es parte de la pregunta.
  await audit.registrar({ tabla: 'conductor', id: Number(id), usuarioId,
    cambios: [{ campo: 'pin_ballenoil', antes: antes ? antes.pin_ballenoil : null, ahora: p.slice(0, 40) }],
  }).catch(() => {});
  return r.rows[0];
}

/**
 * LA LISTA DE BALLENOIL: a quién se le puede poner un PIN.
 *
 * Dos montones en uno:
 *
 *   1. Quien está OPERATIVO: de alta y sin una ausencia encima. Fuera
 *      vacaciones, bajas, permisos y suspendidos —no van a repostar—, que es lo
 *      que la pantalla dice desde siempre.
 *
 *   2. Cuentas de BOLT que aún no son de nadie. Están para verlas, no para
 *      darles PIN: sin ficha no hay a quién guárdarselo. Salen marcadas
 *      `conFicha: false` y la pantalla lo pinta en ámbar.
 *
 * Antes el segundo montón salía del padrón en una hoja y darle el PIN creaba la
 * ficha al vuelo, en otra hoja. Crear a alguien sin DNI, sin contrato y sin
 * turno solo porque tiene cuenta en BOLT es como se llega a una plantilla con
 * fichas a medias; para eso está Selección.
 */
async function paraBallenoil() {
  const r = await db.consulta(
    `SELECT c.id,
            COALESCE(NULLIF(btrim(c.nombre_bolt), ''),
                     btrim(COALESCE(c.apellidos || ', ', '') || c.nombre)) AS nombre,
            NULLIF(btrim(c.nombre_bolt), '')  AS id_bolt,
            tel.e164   AS telefono,
            c.dni_nie, c.email,
            t.etiqueta AS turno,
       ...
       LEFT JOIN LATERAL (
         SELECT th.turno_id FROM conductor_turno_hist th
          WHERE th.conductor_id = c.id AND th.desde <= CURRENT_DATE
            AND (th.hasta IS NULL OR th.hasta >= CURRENT_DATE)
          ORDER BY th.desde DESC LIMIT 1) tt ON TRUE
       LEFT JOIN turno t ON t.id = tt.turno_id
      WHERE NOT c.es_centinela
        AND COALESCE(ce.es_ausencia, false) = false
      ORDER BY nombre`);

  const libres = await db.consulta(
    `SELECT nombre_en_bolt, externo_telefono, externo_email
       FROM v_bolt_libres WHERE estado_externo = 'active'
      ORDER BY nombre_en_bolt`);

  return [
    ...r.rows.map(x => ({
      conductorId: String(x.id), idBolt: x.id_bolt || '', nombre: x.nombre,
      telefono: x.telefono || '', dni: x.dni_nie || '', email: x.email || '',
      turno: x.turno || '', estado: x.estado,
      pin: x.pin_ballenoil || '', obs: x.obs_ballenoil || '', conFicha: true,
    })),
    ...libres.rows.map(x => ({
      conductorId: null, idBolt: x.nombre_en_bolt || '', nombre: x.nombre_en_bolt || '',
      telefono: x.externo_telefono || '', dni: '', email: x.externo_email || '',
      turno: '', estado: '', pin: '', obs: '', conFicha: false,
    })),
  ];
}
/** El PIN de un teléfono. Lo pide el bot cuando el conductor da al botón. */
async function pinPorTelefono(telefono) {
  const t = String(telefono || '').replace(/\D/g, '');
  if (t.length < 9) return null;
  const r = await db.consulta(
    `SELECT c.id, c.pin_ballenoil, c.obs_ballenoil,
            COALESCE(NULLIF(btrim(c.nombre_bolt), ''),
      ...
      LIMIT 1`, [t]);
  const x = r.rows[0];
  return x ? { conductorId: String(x.id), quien: x.quien,
               pin: x.pin_ballenoil || '', obs: x.obs_ballenoil || '' } : null;
}
module.exports = {
