...
const gen = require('./generador.service');
// Por el SERVICIO de vacantes, no por su repositorio: desde fuera de un fichero
// de dominio se entra por la puerta, aunque el vecino viva en el mismo módulo.
...
router.get('/api/vacantes', responde(async req =>
  ({ vacantes: await vacantes.listar({ incluirCerradas: req.query.cerradas === '1' }) })));
...
router.delete('/api/vacante/:id', responde(async req =>
  vacantes.eliminar(req.params.id, await quien(req))));
