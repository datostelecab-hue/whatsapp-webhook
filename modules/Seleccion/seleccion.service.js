...
// No son de la persona sino del DOCUMENTO del permiso. La regla —dónde van, qué
// se comprueba— vive en Documentos, que la usa también Plantilla.
...
async function guardar(id, datos = {}, quien = {}) {
  ...
  if (hay(exp) || hay(cad)) {
    const f = await cand.ficha(Number(id));
    if (!f) throw new Error('No existe esa candidatura');
    fechas = await docs.prepararFechasCarne(f.conductor_id, { expedicion: exp, caducidad: cad });
  }
